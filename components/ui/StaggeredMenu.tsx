...
interface StaggeredMenuProps {
  position?: 'left' | 'right';
  colors?: string[]; // Array of colors for pre-layers
  items?: MenuItem[];
...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  position = 'right',
  colors = ['#E5D0D4', '#D9A9B0', '#FAF7F7'], // Palette Rose
  items = [],
  socialItems = [],
  displaySocials = true,
  displayItemNumbering = true,
  className = '',
  menuButtonColor = '#fff', // Mix-blend handles visibility, but default white for difference
  openMenuButtonColor = '#754548',
  accentColor = '#754548',
  ...
  const buildOpenTimeline = useCallback(() => {
    ...
    layerStates.forEach((ls, i) => {
      tl.fromTo(ls.el, { xPercent: ls.start }, { xPercent: 0, duration: 0.6, ease: 'power4.inOut' }, i * 0.08);
    });
    ...
    tl.fromTo(
      panel,
      { xPercent: panelStart },
      { xPercent: 0, duration: panelDuration, ease: 'power4.inOut' },
      panelInsertTime
    );

    if (itemEls.length) {
      const itemsStartRatio = 0.4;
  ...
  const animateColor = useCallback(
    (opening: boolean) => {
      ...
      if (changeMenuColorOnOpen) {
        // NOTE: Since we are using mix-blend-mode difference, 
        // we might want to keep white/light colors for visibility against light backgrounds
  ...
  const animateText = useCallback((opening: boolean) => {
    ...
    const cycles = 2; // Reduced cycles for snappier feel
            ...
            {items && items.length > 0 ? (
              ...
              <li className="sm-panel-itemWrap" aria-hidden="true">
                <span className="sm-panel-item">
                  <span className="sm-panel-itemLabel">Empty</span>
                </span>
              </li>
            )}
          </ul>
