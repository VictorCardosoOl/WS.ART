    ...
    <section ref={sectionRef} id="services" data-theme="dark" className="py-32 md:py-48 text-stone-100 overflow-hidden relative transition-colors duration-700">
      <div className="w-full max-w-[1920px] mx-auto px-5 md:px-12 lg:px-20 relative z-20">
        ...
        <div className="flex flex-col lg:flex-row gap-20 lg:gap-32 relative">
          ...
          {/* Floating Image Preview (Desktop Only) - Optimized with GSAP */}
          <div className="hidden lg:block w-1/3 relative pointer-events-none">
            <div ref={imageContainerRef} className="sticky top-40 w-full aspect-[3/4] overflow-hidden rounded-sm bg-stone-900">
              {SERVICES_ITEMS.map((service, index) => (
                <img
                  key={index}
                  src={serviceImages[index] || service.image}
                  alt={service.title}
                  className="absolute inset-0 w-full h-full object-cover grayscale opacity-0 will-change-opacity"
