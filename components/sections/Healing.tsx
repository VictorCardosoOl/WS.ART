        ...
        <section className="py-24 md:py-32 bg-[#FAF7F7] relative overflow-hidden" id="healing">
            ...
            <div className="w-full max-w-[1920px] mx-auto px-6 md:px-12 lg:px-24 relative z-10">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-24 relative">
                        ...
                        <div className="sticky top-32 flex flex-col gap-4">
                            {phases.map((phase) => (
                                <button
                                    ...
                                    className={`group relative text-left p-6 transition-all duration-500 border-l-2 ${activePhase === phase.id
                                ...
                                >
                                    <span className={`block text-[10px] font-bold uppercase tracking-widest mb-1 transition-colors ${activePhase === phase.id ? 'text-[#754548]' : 'text-stone-400'
                                        }`}>
                                        Fase {String(phase.id).padStart(2, '0')}
                                    </span>
                                    <span className={`block font-serif text-2xl md:text-3xl transition-colors ${activePhase === phase.id ? 'text-stone-900' : 'text-stone-400 group-hover:text-stone-600'
                                        }`}>
                                        {phase.days}
                                    </span>

                                    {/* Active Indicator Icon */}
                                    <div className={`absolute right-6 top-1/2 -translate-y-1/2 transition-all duration-500 ${activePhase === phase.id ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4'
                                        }`}>
                    ...
                    {/* Right: Content Display (GSAP Animation) */}
